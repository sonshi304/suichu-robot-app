...
function Who({ activities, reservations, members }: {
  ...
  const t = todayStr();
  const upcoming = Object.keys(activities).filter((d) => d >= t).sort().slice(0, 14);
  ...
  return (
    <div className="page">
      ...
      {upcoming.map((d) => {
        const dr = reservations[d] || [];
        return (
          <div key={d} className="card">
            <div className="flex-between mb-2">
              <div className="fw-700">{formatDate(d)} — {activities[d]?.title || "通常活動"}</div>
              <div className="text-xs text-hint">{activities[d]?.time || "放課後"}</div>
            </div>
            <div className="text-xs text-muted mb-3">参加予定: {dr.length}人 / {members.length}人</div>
            {dr.length > 0 ? (
              <div className="flex-col" style={{ gap: 4 }}>
                {dr.map((id) => {
                  const m = members.find((x) => x.id === id);
                  return m ? (
                    <div key={id} className="member-row" style={{ padding: "6px 0" }}>
            ...
            ) : (<div className="text-hint text-xs" style={{ padding: "8px 0" }}>まだ予約がありません</div>)}
          </div>
        );
      })}
    </div>
